import { Component } from '@angular/core';
import { DatePipe } from '@angular/common';
import { NavController, NavParams } from 'ionic-angular';
import {SettingsPage} from '../settings/settings';
import {EndPoint} from '../../providers/end-point';
import {LoadingController,ToastController,AlertController} from 'ionic-angular';
import { UserProvider } from '../../providers/user-provider';
import { Storage } from '@ionic/storage';
import { OauthProvider } from '../../providers/oauth-provider';
@Component({
  selector: 'page-subscription-edit',
  templateUrl: 'subscription-edit.html'
})
export class SubscriptionEditPage {
  flag:any;
  subscription:any;
   myStartDate:any;
  myEndDate:any  ;
  request:any = {};
  user:any = {};
  myStartDateUnformat:any;
  myEndDateUnformat:any;
  dPickerStartDate ="";
  dPickerEndDate ="";
  preferenceExtreme ={
      notification_types:[],
      severity:"extreme"
    }
    preferenceHigh ={
      notification_types:[],
      severity:"high"
    }
    preferenceMedium ={
      notification_types:[],
      severity:"medium"
    }
    preferenceLow ={
      notification_types:[],
      severity:"low"
    }
    preferenceMinimal ={
      notification_types:[],
      severity:"minimal"
    }
    pushExtreme 
    pushHigh 
    pushMedium 
    pushLow 
    pushMinimal

    emailExtreme
    emailHigh 
    emailMedium 
    emailLow
    emailMinimal 

  showDate = true;
  showDateEnd = true;
  categories: any[];



  crime_check = false;
  civil_unrest_check = false;
  travel_transport_check = false;
  terrorism_check = false;
  political_check = false;
  conflict_check = false;
  natural_hazard_check = false;
  kidnapping_check = false;
  infrastructure_check = false;
  unnatural_hazard_check = false;
  piracy_check = false;
  health_check = false;
  risk_rating_check = false;
  city_name:any;
  country_name:any;


  showSeverities =true;
   showDates =true
    showCategories =true
    preferences:any;

  constructor(public navCtrl: NavController, 
  public navParams: NavParams,
  public endPoint:EndPoint,
     public loading:LoadingController,
     public toast:ToastController,
     public alert: AlertController,
     public datePipe: DatePipe,
     public userProvider:UserProvider,
     public storage:Storage,
     public oauth:OauthProvider
  
  ) {

          this.subscription = this.navParams.get('subscription');
          console.log(this.subscription.subData);
          this.categories = this.subscription.subData.categories;
          this.preferences = this.subscription.subData.preferences;
          this.country_name = this.navParams.get('countryName');
          this.city_name = this.navParams.get('city_name');
          this.flag = this.navParams.get('countryFlag');
          this.myStartDate = this.subscription.subData.subs_start.substring(0,10);
          this.myEndDate = this.subscription.subData.subs_end.substring(0,10);

           for(let i=0; i< this.preferences.length;i++){
                if(this.preferences[i].severity=="extreme"){

                    if(this.preferences[i].notification_types.indexOf('push') > -1){
                            this.pushExtreme = true
                    }
                    if(this.preferences[i].notification_types.indexOf('email') > -1){
                        this.emailExtreme = true
                    }
                }
                if(this.preferences[i].severity=="high"){
                    if(this.preferences[i].notification_types.indexOf('push') > -1){
                        this.pushHigh = true
                    }
                    if(this.preferences[i].notification_types.indexOf('email') > -1){
                        this.emailHigh = true
                    }
                }

                if(this.preferences[i].severity=="medium"){
                    if(this.preferences[i].notification_types.indexOf('push') > -1){
                        this.pushMedium = true
                    }
                    if(this.preferences[i].notification_types.indexOf('email') > -1){
                        this.emailMedium = true
                    }
                }

                if(this.preferences[i].severity=="low"){
                    if(this.preferences[i].notification_types.indexOf('push') > -1){
                        this.pushLow = true
                    }
                    if(this.preferences[i].notification_types.indexOf('email') > -1 ){
                        this.emailLow = true
                    }
                }

                if(this.preferences[i].severity=="minimal"){
                    if(this.preferences[i].notification_types.indexOf('push') > -1){
                        this.pushMinimal = true
                    }
                    if(this.preferences[i].notification_types.indexOf('email') > -1 ){
                        this.emailMinimal = true
                    }
                }

            }

          

          

          

          
          

  }
   showToast() {
    let toast = this.toast.create({
      message: 'You have changed your subscription',
      duration: 3000,
      showCloseButton: false,
      dismissOnPageChange: false,
      cssClass: "toast",
    });
    toast.present();
  }

  transformDate(dateStart, dateEnd){
        console.log(dateStart);
        console.log(dateEnd);
        
      let newDateStart = this.datePipe.transform(dateStart)
      let newDateEnd = this.datePipe.transform(dateEnd)
        console.log(newDateStart);
        console.log(newDateEnd);
        
        this.myStartDate = newDateStart;
        this.myEndDate = newDateEnd;

        
  }
  changeVal(event){
    console.log(event);
    this.showDate = false;

  }

  changeValEnd(event){
    console.log(event);
    this.showDateEnd = false;

  }
  editLocale(format){

   for(let i=0; i < this.endPoint.subscriptions.length; i++){

            if(this.endPoint.subscriptions[i].id == format.id){

                  this.endPoint.subscriptions[i] = format;
                  

            }
      }

      
  }
 

  editSubscription(){
    /*  if(this.dPickerStartDate == "" ){
            this.dPickerStartDate = this.myStartDate.substring(0 , this.myStartDate.length -9 );
      } if(this.dPickerEndDate == ""){
            this.dPickerEndDate = this.myEndDate.substring(0 , this.myEndDate.length -9 );
      }
 */

    this.request.country_iso = this.subscription.subData.country_iso;
    (this.city_name != undefined)? this.request.city_name = this.city_name : null;
      this.user = this.userProvider.user;
      this.request.user = this.user;
      this.request.subs_start= this.myStartDate+" 00:00:00";
      this.request.subs_end= this.myEndDate+" 00:00:00";

      console.log("this.request.subs_start",this.request.subs_start);

      this.request.categories = this.categories;
      this.request.preferences= [];
       if(this.pushExtreme == true || this.emailExtreme == true ){
          this.preferenceExtreme.notification_types= [];
          if(this.pushExtreme == true){
              this.preferenceExtreme.notification_types.push('push');
          }
          if(this.emailExtreme == true){
             this.preferenceExtreme.notification_types.push('email'); 
          }
          this.request.preferences.push(this.preferenceExtreme)
          
      }

      if(this.pushHigh == true || this.emailHigh == true ){
          this.preferenceHigh.notification_types= [];
          if(this.pushHigh == true){
              this.preferenceHigh.notification_types.push('push');
          }
          if(this.emailHigh == true){
             this.preferenceHigh.notification_types.push('email'); 
          }
          this.request.preferences.push(this.preferenceHigh)
          
      } 

      if(this.pushMedium == true || this.emailMedium == true ){
          if(this.pushMedium == true){
              this.preferenceMedium.notification_types.push('push');
          }
          if(this.emailMedium == true){
             this.preferenceMedium.notification_types.push('email'); 
          }
          this.request.preferences.push(this.preferenceMedium)
          
      }
      if(this.pushLow == true || this.emailLow == true ){
          if(this.pushLow == true){
              this.preferenceLow.notification_types.push('push');
          }
          if(this.emailLow == true){
             this.preferenceLow.notification_types.push('email'); 
          }
          this.request.preferences.push(this.preferenceLow)
          
      }
      if(this.pushMinimal == true || this.emailMinimal == true ){
          if(this.pushMinimal == true){
              this.preferenceMinimal.notification_types.push('push');
          }
          if(this.emailMinimal == true){
             this.preferenceMinimal.notification_types.push('email'); 
          }
          this.request.preferences.push(this.preferenceMinimal)
          
      }
  
      this.request.created_by = "user";
      console.log(this.request);
      

     let loading = this.loading.create({
      
        });
      loading.present();
      this.oauth.getToken().then(token=>{
                     this.endPoint
        .editSubscription(this.subscription.id,this.request,token)
        .then(count=>{ 
              console.log(count);
              
              let format_save = {id: count.message, subData: this.request}
             this.editLocale(format_save);
             this.navCtrl.setRoot(SettingsPage);
              
              
              loading.dismiss();
           
        });

      })
      
 

  }

  changeSeverities(){
   this.showSeverities = !this.showSeverities;
  }
  changeDates(status){
     this.showDates = !this.showDates;
  }
  changeCategories(status){
      this.showCategories = !this.showCategories;
  }


  showCategoriesAlert(){
    let alert = this.alert.create();
    alert.setTitle('Categories');
    
    this.crime_check = false;
    this.civil_unrest_check = false;
    this.travel_transport_check = false;
    this.terrorism_check = false;
    this.political_check = false;
    this.conflict_check = false;
    this.natural_hazard_check = false;
    this.kidnapping_check = false;
    this.infrastructure_check = false;
    this.unnatural_hazard_check = false;
    this.piracy_check = false;
    this.health_check = false;
    this.risk_rating_check = false;


      for(let category of this.categories){

      if(category == "civil_unrest"||category == "civil unrest"){

                this.civil_unrest_check = true;

          }

          if(category == "crime"){

              this.crime_check = true;


          }

          if(category == "travel transport"){

               
              this.travel_transport_check = true;

          }

           if(category == "terrorism"){

                this.terrorism_check = true;

          }

          if(category == "political"){

                this.political_check = true;

          }
          if(category == "conflict"){

                this.conflict_check = true;

          }

          if(category == "natural hazard"){

                this.natural_hazard_check = true;

          }
          if(category == "unnatural hazard"){

                this.unnatural_hazard_check = true;

          }
          
          if(category == "piracy"){

                this.piracy_check = true;

          }

          if(category == "health"){

                this.health_check = true;

          }

          if(category == "infrastructure"){

                this.infrastructure_check = true;

          }
          if(category == "kidnapping"){

                this.kidnapping_check = true;

          }

          if(category == "risk rating"){

                this.risk_rating_check = true;

          }

          

      }

      alert.addInput({
                      type: 'checkbox',
                      label: 'Civil Unrest',
                      value: 'civil_unrest',
                      checked: this.civil_unrest_check
                      ,disabled:this.subscription.subData.created_by=='tds'
      });

       alert.addInput({
                      type: 'checkbox',
                      label: 'Conflict',
                      value: 'conflict',
                      checked: this.conflict_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      alert.addInput({
                      type: 'checkbox',
                      label: 'Crime',
                      value: 'crime',
                      checked: this.crime_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

       alert.addInput({
                      type: 'checkbox',
                      label: 'Health',
                      value: 'health',
                      checked: this.health_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      alert.addInput({
                      type: 'checkbox',
                      label: 'Infrastructure',
                      value: 'infrastructure',
                      checked: this.infrastructure_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

       alert.addInput({
                      type: 'checkbox',
                      label: 'Kidnapping',
                      value: 'kidnapping',
                      checked: this.kidnapping_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      alert.addInput({
                      type: 'checkbox',
                      label: 'Natural Hazard',
                      value: 'natural hazard',
                      checked: this.natural_hazard_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      alert.addInput({
                      type: 'checkbox',
                      label: 'Piracy',
                      value: 'piracy',
                      checked: this.piracy_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      alert.addInput({
                      type: 'checkbox',
                      label: 'Risk Rating',
                      value: 'risk rating',
                      checked: this.risk_rating_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      alert.addInput({
                      type: 'checkbox',
                      label: 'Unnatural Hazard',
                      value: 'unnatural hazard',
                      checked: this.unnatural_hazard_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });

      

      

       alert.addInput({
                      type: 'checkbox',
                      label: 'Terrorism',
                      value: 'terrorism',
                      checked: this.terrorism_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });
      

      alert.addInput({
                      type: 'checkbox',
                      label: 'Travel Transport',
                      value: 'travel transport',
                      checked: this.travel_transport_check
                       ,disabled:this.subscription.subData.created_by=='tds'
      });


    

     

    

    alert.addButton('Cancel');
    alert.addButton({
      text: 'Ok',
      handler: data => {
        this.categories = data;
        console.log('Checkbox data:', data);
      }
    });
    alert.present();
  }
      }
    